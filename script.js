...
const themes = {
    dark: {
        ...
        border: 0x0a0a0a,
        wall: 0x222222,
        texturePath: './datacenter-green.png',
    ...
    blue: {
        ...
        border: 0x0a0a1a,
        wall: 0x222244,
        texturePath: './datacenter-blue.png',
        instructionColor: 'white',
        textShadow: 'none'
    },
    whitegray: {
        ...
        border: 0xaaaaaa,
        wall: 0xdddddd,
        texturePath: './datacenter-gray.png',
        instructionColor: '#0a0a0a',
        textShadow: '-1px -1px 0 #fff, 1px -1px 0 #fff, -1px 1px 0 #fff, 1px 1px 0 #fff'
    }
}
const themeKeys = Object.keys(themes);
const activeTheme = themes[themeKeys[Math.floor(Math.random() * themeKeys.length)]];
...
function setup() {
    ...
    scene.fog = new THREE.FogExp2(activeTheme.fog, 0.15)
    light = new THREE.AmbientLight(activeTheme.lightColor, activeTheme.lightIntensity);
    scene.add(light);
...
function lightThrob(toIntensity) {
    let interval = 150
    let step = (toIntensity - activeTheme.lightIntensity) / interval
    let curIntensity = activeTheme.lightIntensity
...
const floorMat1 = new THREE.MeshPhongMaterial({color: activeTheme.floor1});
const floorMat2 = new THREE.MeshPhongMaterial({color: activeTheme.floor2});
function buildFloor() {
...
const roofMat = new THREE.MeshPhongMaterial({color: activeTheme.roof});
function buildRoof() {
    for (let x = 0; x < floorLength; x++) {
        for (let z = 0; z < floorWidth; z++) {
...
function buildBorders() {
    const bMat = new THREE.MeshPhongMaterial({color: activeTheme.border, wireframe: false});
...
const wallTexture = new THREE.TextureLoader().load(activeTheme.texturePath);
const wallMat = new THREE.MeshPhongMaterial({map: wallTexture, wireframe: false});
...
function buildWalls() {
    for (let i = 0; i < nofWalls; i++) {
        const wallType = Math.floor(Math.random() * nofWallTypes) + 1
        const x = Math.floor(Math.random() * floorLength)
        const z = Math.floor(Math.random() * floorWidth)
        switch (wallType) {
            case 1: {  // 1x1
...
function spawn() {
    let x = Math.floor(Math.random() * floorLength)
    let z = Math.floor(Math.random() * floorWidth)
    while (grid[z][x].isWall) {
        x = Math.floor(Math.random() * floorLength)
        z = Math.floor(Math.random() * floorWidth)
    }
...
const texture_message = [  // Texture, Death Message
    ['farhad', 'You were caught by Farhad. The datacenter is now your home.']
]
const Farhads_TargetSquare_Path = []  // [i] = { OoHoo: mesh, TargetSquare: [x,z], Path: [[x, z],...] }
const OooHooGeo = new THREE.BoxGeometry(1.2, 1.2, 0.01)

function generateFarhads() {
    for (let i = 0; i < texture_message.length; i++) {
        let texture = textureLoader.load('./' + texture_message[i][0] + '.png')
        let OoHoo = new THREE.Mesh(OooHooGeo, new THREE.MeshPhongMaterial({map: texture, transparent: true}))
        Farhads_TargetSquare_Path.push({OoHoo: OoHoo, TargetSquare: [null, null], Path: []})
...
function setTargetSquare(i) {
    ...
    const TargetSquare = Farhads_TargetSquare_Path[i].TargetSquare
    if (Farhads_TargetSquare_Path[i].OoHoo.position.distanceTo(camera.position) < 30) {
        ...
        if (Farhads_TargetSquare_Path[i].Path.length === 0) {
            let x = Math.floor(Math.random() * floorLength)
            let z = Math.floor(Math.random() * floorWidth)
            while (grid[z][x].isWall) {
                x = Math.floor(Math.random() * floorLength)
...
function findPath(i) {
    const OoHoo = Farhads_TargetSquare_Path[i].OoHoo
    const TargetSquare = Farhads_TargetSquare_Path[i].TargetSquare
    ...
    Farhads_TargetSquare_Path[i].Path = path
}
function stepFarhad(i) {
    const OoHoo = Farhads_TargetSquare_Path[i].OoHoo
    const Path = Farhads_TargetSquare_Path[i].Path
...
function endGame(OoHoo) {
    ...
    if (alertMessage) {
        ...
        alertMessage.style.opacity = 0; // Close alert immediately
    }
    ...
    let interval = setInterval(() => {
        ...
        if (i === nofIterations) {
            ...
            light.intensity = activeTheme.lightIntensity
            for (let i = 0; i < 500; i++) {
                setTimeout(() => {
...
async function tickFarhads() {
    if (!gameOver) {
        for (let i = 0; i < Farhads_TargetSquare_Path.length; i++) {
            stepFarhad(i)
            if (tick % 200 === 0) {
...
let started = false
function main() {
    document.getElementById('start').style.color = activeTheme.instructionColor;
    document.getElementById('start').style.textShadow = activeTheme.textShadow;
    spawn()
    renderer.render(scene, camera)
...
function checkProximity() {
    ...
    let isClose = false;
    for (let i = 0; i < Farhads_TargetSquare_Path.length; i++) {
        if (Farhads_TargetSquare_Path[i].OoHoo.position.distanceTo(camera.position) < 7) {
            isClose = true;
            break;
        }
    }
...
function startLightFlickering() {
    setInterval(() => {
        if (!gameOver && Math.random() > 0.9) {
            let originalIntensity = light.intensity;
            light.intensity = Math.max(0.1, originalIntensity - 0.5);
            setTimeout(() => {
                if (!gameOver) light.intensity = originalIntensity;
            }, 100);
        }
    }, 3000);
}
